...
PROVENANCE_VIEW_NAME = "Generated by SpeciesGenerator"
...
def add_provenance_view(fdb_path):
    """Add an inert, human-readable marker to a generated SQLite FDB."""
    con = sqlite3.connect(fdb_path)
    try:
        con.execute(
            'CREATE VIEW IF NOT EXISTS "Generated by SpeciesGenerator" AS '
            'SELECT \'Generated by SpeciesGenerator\' AS "Provenance"')
        con.commit()
    finally:
        con.close()
...
def allocate_species_ids(species_configs, fdb_path=None, floor=None,
    ...
    try:
        ...
        used_sids.update(int(value) for value in (reserved_species_ids or ()))
        used_gids = set(source_gids)
        used_gids.update(int(value) for value in (reserved_genetic_ids or ()))
